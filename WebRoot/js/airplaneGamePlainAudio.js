...
(function($) {
	...
	// 游戏配置
	var config = {
		"imageSrc" : "../img/airplaneGame/", // 图片url前缀
		"soundSrc" : "../airplaneSounds/", // 声音url前缀
		"loadImg" : [ 'bg.jpg', 'loading1.png', 'loading2.png', 'loading3.png',
				'logo.png' ], // 等待动画图片资源
		"gameImg" : [ 'cartridge.png', 'cartridge_power.png', 'die1.png',
				'die2.png', 'me.png', 'plain1.png', 'plain2.png', 'plain3.png',
				'plain2_hurt.png', 'plain3_hurt.png', 'plain1_die1.png',
				'plain1_die2.png', 'plain1_die3.png', 'plain2_die1.png',
				'plain2_die2.png', 'plain2_die3.png', 'plain2_die4.png',
				'plain3_die1.png', 'plain3_die2.png', 'plain3_die3.png',
				'plain3_die4.png', 'plain3_die5.png', 'plain3_die6.png',
				'me_die1.png', 'me_die2.png', 'me_die3.png', 'me_die4.png',
				'prop1.png', 'prop2.png', 'bomb.png', 'me_2.png',
				'plain3_2.png' ], // 游戏图片资源
		"gameSound" : [ 'achievement.mp3', 'plain2_die.mp3', 'plain3_die.mp3',
				'fire_bullet.mp3', 'game_music.mp3', 'game_over.mp3',
				'get_bomb.mp3', 'get_double_laser.mp3', 'show_prop.mp3',
				'plain1_die.mp3', 'use_bomb.mp3' ],
		"gameSpeed" : 8, // 游戏速度
		"cartridgeSpeed" : 6, // 子弹速度
		"isMobile" : navigator.userAgent
				.match(/(iPhone|iPod|iPad|Android|BlackBerry|mobile)/), // 是否手机
		"isPhone" : navigator.userAgent.match(/(iPhone|iPod|iPad)/), // 是否使用safari浏览器
	};
	...
	function creatImg(src) {
		...
		source[src].src = config.imageSrc + src;// 将图片资源存放在source中
		return source[src];
	}
	...
	function creatAudio(src) {
		...
		if (safari==84 || safari==-1) {
		} else {
			source[src] = new Audio();
			source[src].src = config.soundSrc + src;// 将声音资源存放在source中
			return source[src];
		}
	...
	function loading() {
		...
		function load() {
			...
			var pic = creatImg("loading" + (parseInt(loadingTime / 200) + 1)
					+ ".png");
		...
		if (userAgent.indexOf("Safari")==84) {
	...
	function game() {
		...
		player.move = function(x, y) {
			...
			player.x = player.x > c_width - player.width ? c_width
					- player.width : player.x;
			player.x = player.x < 0 ? 0 : player.x;
			player.y = player.y > c_height - player.height ? c_height
		...
		player.moveing = function() {
			...
			cxt.drawImage(game.time % 30 > 15 ? player.model : player.model2,
		...
		player.attack = function() {
			...
			player.attackTime++;
			if ((player.attackTime * game.refreshInterval)
					% (game.refreshInterval * 20) != 0) {
				return;
			}
			...
			if (player.attackPower) {
				cartridges = [
						(new cartridge(player.x - (player.width / 5), player.y,
								2, player)),
						(new cartridge(player.x + (player.width / 5), player.y,
								2, player)) ];
			} else {
		...
		player.attacking = function() {
			...
			firstloop: for ( var i = cartridges_length; i--;) {
				...
				cxt.drawImage(cartridge.model, cartridge.x, cartridge.y,
				...
				secondloop: for ( var j = plain_length; j--;) {
					...
					if (X > plain.x && X < (plain.x + plain.width)
		...
		player.die = function() {
			...
			function playerDie() {
				...
				this.call = function() {
					...
					var dieModel = creatImg("me_die"
							+ (parseInt((dieTime - this.animationTime)
									/ dieSpeed) + 1) + ".png");
		...
		game.bgScroll = function() {
			...
			cxt.drawImage(game.bgImg, 0, game.bgScrollTime - bg_img_height,
					bg_img_width, bg_img_height);
			cxt.drawImage(game.bgImg, 0, game.bgScrollTime, bg_img_width,
					bg_img_height);
		};
		game.props = [];
		game.addProp = function() {
			...
			if ((game.time * game.refreshInterval) % (interval * 1000) == 0) {
				game.props
						.push((new prop(parseInt(Math.random() * 1.8 + 1.1))));
				if (safari==-1 || safari==84) {
				} else
		...
		game.plainsScroll = function() {
			...
			function isCollide(plain) {
				...
				var plainBottomRight = [ plain.x + plain.width,
						plain.y + plain.height ];
				var meTopLeft = [ game.player.x + game.player.width / 3,
						game.player.y ];
				var meBottomRight = [
						game.player.x + (game.player.width * 2 / 3),
						game.player.y + (game.player.height * 2 / 3) ];

				var collideTopLeft = [ Math.max(plainTopLeft[0], meTopLeft[0]),
						Math.max(plainTopLeft[1], meTopLeft[1]) ];
				var collideBottomRight = [
						Math.min(plainBottomRight[0], meBottomRight[0]),
						Math.min(plainBottomRight[1], meBottomRight[1]) ];

				if (collideTopLeft[0] < collideBottomRight[0]
						&& collideTopLeft[1] < collideBottomRight[1]) {
					return true;
				}
		...
		game.refreshMessage = function() {
			...
			if (game.player.bomb > 0) {
				...
				cxt.drawImage(bombModel, 10, c_height - bombModel.height - 10,
						bombModel.width, bombModel.height);
				cxt.fillText(game.player.bomb, 20 + bombModel.width, c_height
						- bombModel.height + 28);
			}
		};
		game.start();
		...
		function prop(type) {
			...
			this.show = function() {
				...
				cxt.drawImage(this.model, this.x, this.y, this.width,
						this.height);
				if (isGain(this)) {
					this.isDeleted = true;
					this.byGain();
			...
			var isGain = function(prop) {
				...
				if (rightX < game.player.x
						|| leftX > (game.player.x + game.player.width)) {
					return false;
				}
				var removing = prop.status == "move" ? prop.speed
						: (prop.showType == "down" ? prop.speed : -prop.speed);
				var nextY = prop.y + removing;
				if (((prop.y + prop.height) > game.player.y || (nextY + prop.height) < game.player.y)
						&& game.player.lastY > (prop.y + prop.height)) {
					return false;
				}
				return true;
			};
		...
		function plain(type) {
			...
			this.speed = this.speed < 0.5 ? Math.random() * 0.5 + 0.5
					: this.speed;
			this.speed = this.speed > this.maxSpeed ? this.maxSpeed
					: this.speed;

			this.show = function() {
				if (this.type == 3) {
					cxt.drawImage(game.time % 30 > 15 ? this.model
							: this.model2, this.x, this.y, this.width,
							this.height);
					return;
				}
				cxt.drawImage(this.model, this.x, this.y, this.width,
		...
		function cartridge(x, y, type, player) {
			this.model = creatImg(type == 2 ? "cartridge_power.png"
					: "cartridge.png");
		...
		$("#modal").on("click", ".playAgain", game.start);
	}
})(jQuery);